...
-- A better implementation with efficient primeness check:
-- We only consider odd numbers, check only up to sqrt(x),
-- and use the fact that every prime above 3 can be written as 6k +/- 1.
problem7 :: Int -> Int
problem7 n = primes!!(n-1)
  where
    primes    = 2 : 3 : filter isPrime [5,7..]
    isPrime x = if x `mod` 3 == 0 then False else go 5
      where
        go k
            |k^2 > x            = True
            |x `rem` k == 0     = False
